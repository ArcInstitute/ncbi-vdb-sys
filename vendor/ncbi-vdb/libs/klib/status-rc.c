...
static char const *Get_RC_String(size_t *const outsize, int const value,
                                 char const *const *const strings,
                                 int const limit) {
  char const *const result =
      (0 <= value && value < limit) ? strings[value] : INVALID;
...
static void Get_RC_String_Module(struct RC_String *out, rc_t const rc) {
  ...
  out->text = Get_RC_String(&out->size, out->value = (int)GetRCModule(rc),
                            RCModuleStrings, rcLastModule_v1_2);
}
...
static void Get_RC_String_Context(struct RC_String *out, rc_t const rc) {
  ...
  out->text = Get_RC_String(&out->size, out->value = (int)GetRCContext(rc),
                            RCContextStrings, rcLastContext_v1_1);
}
...
static void Get_RC_String_State(struct RC_String *out, rc_t const rc) {
  out->field = rcf_state;
  out->text = Get_RC_String(&out->size, out->value = (int)GetRCState(rc),
                            RCStateStrings, rcLastState_v1_1);
}
...
size_t KWrtFmt_rc_t(char *pout, size_t max, const char *fmt, rc_t rc_in) {
  ...
  if (*fmt == '#')
...
#if _DEBUGGING
                       "RC(%s:%u:%s %*s,%*s,%*s,%*s,%*s)", GetRCFilename(),
                       GetRCLineno(), GetRCFunction(),
#else
                       "RC(%*s,%*s,%*s,%*s,%*s)",
#endif
    ...
    rc = string_printf(pout, max, &needed, "RC(%*s)", (uint32_t)state_size,
                       state_str);
  }
